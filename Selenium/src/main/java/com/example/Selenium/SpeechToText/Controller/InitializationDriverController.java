...
//                    dataStoreModel.setCountDownLatch(new CountDownLatch(1));
//                    Thread threadCheckHandAd = new Thread(new CheckHandAdController(driver, dataStoreModel.getCountDownLatch()));
//                    threadCheckHandAd.start();
//                    dataStoreModel.getCountDownLatch().await();
    ...
    public void changeFileName() {
        ...
        if (files != null && files.length > 0) {
            ...
            if (textCsvStoreModel.isFlag()) {
                if (latestFile.getName().equals(dataStoreModel.getParams().get("FileName") + ".mp3")) {
                    String newFileName = dataStoreModel.getParams().get("FileName") + " (1) .mp3";
